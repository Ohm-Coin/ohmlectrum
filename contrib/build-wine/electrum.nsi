...
;Variables
  ...
  !define PRODUCT_NAME "Ohmlectrum"
  !define PRODUCT_WEB_SITE "https://github.com/pooler/ohmlectrum"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/ohmlectrum-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\ohmlectrum-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\ohmlectrum-${PRODUCT_VERSION}.exe" "" "$INSTDIR\ohmlectrum-${PRODUCT_VERSION}.exe" 0

  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\ohm" "" "URL:ohm Protocol"
  WriteRegStr HKCU "Software\Classes\ohm" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\ohm" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\ohm\shell\open\command" "" "$\"$INSTDIR\ohmlectrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\ohm"
